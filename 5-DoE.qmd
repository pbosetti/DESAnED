...
If the **model of interest is univariate**, the experiment consists of analyzing the response variable at a sequence of levels for the explanatory variable. The number of levels is correlated with the expected degree of the response: for a regression of degree $n$ you need **at least** $n+1$ levels

But if the model is **multivariate**, i.e. the output depends on $m$ explanatory variables, and each variable is investigated on $k$ levels, then the number of test conditions is $k^m$
...
If in a univariate experiment the explanatory variable, or [factor]{.bgreen}, takes on a **sequence** of values, in a **multivariate** experiment the $n$ factors take on an $n$-dimensional **grid** of values, called **factorial plan **
...
In a fractional FP $2^{3-1}$ with defining relation $I=ABC$, we consider these effects:
$$
\begin{align}

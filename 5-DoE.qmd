...
**Industrial experiments** often involve numerous factors corresponding to very complex **multivariate models**

It is therefore necessary to **minimize the number of treatments** and, therefore, the cost of the experiment, and yet obtaining **the same information**
...
::: columns
::: column
A **scientific experiment** is generally conducted for the purpose of supporting or disproving a theory

- it is always based on a **theoretical model** to be verified
- often the model focuses on the effect of a limited number of factors
:::

::: column
In the **industrial field** this is often not possible:

- often a **theoretical model** for the object of the experiment is not available for scientific, technical or practical reasons
- the **interaction** between multiple factors is often what matters most
:::
:::
...
3. **identify** the factors that influence a process: the model may be unknown and we want to determine the [list of factors]{.bgreen} that appear in the $y=f(\cdot)$; the objective is to build an approximate empirical model, which can possibly be used as a starting point for the formulation of a theoretical model

## Dimensionality of an experiment

If the **model of interest is univariate**, the experiment consists of analyzing the output at a sequence of levels for the input. The number of levels is correlated with the expected degree of the response: for a regression of degree $n$ you need **at least** $n+1$ levels

But if the model is **multivariate**, i.e. the output depends on $m$ factors, and each factor is investigated on $k$ levels, then the number of test conditions is $k^m$
...
If in a univariate experiment the factor takes on a **sequence** of values, in a **multivariate** experiment the $n$ factors take on an $n$-dimensional **grid** of values, called **factorial plan **
...
## $k^2$ factorial planes

In general, an experiment in which you have 2 factors each tested on $k$ levels is a $k^2$ factorial design, because the **total number of treatments** is $N=rk^2$, where $r$ is the number of repetitions for each treatment
...
3. conduct of experiments and data collection
...
**It is necessary to evaluate normality and absence of pattern in the residues**
:::
:::column
...
* checks for absence of patterns in the residues depending on the test order
...
* they still allow you to define the sensitivity of the process to the various factors, **excluding non-significant factors**
* are the starting point of any multivariate analysis of complex processes
:::
...
:::aside
Following this transformation it is therefore clear that the new model is more adequate, with pattern-free residues and more compatible with a normal distribution
:::
...
You then choose the value included in the range **e which represents a "convenient" transformation**

For example, if the optimal $\lambda$ was 0.58, I would still choose $\lambda=0.5$, which corresponds to the transformation $y^*=\sqrt{y}$
:::
...
:::aside
Whenever a linear model is **suspicious** (in a **regression** or in **factorial design analysis**) it is a good idea to try a Box-Cox transformation. In PFs, sometimes the Box-Cox transformation can **simplify the model** (i.e. reduce the number of significant factors or interactions)
:::
...
:::columns
:::column
It would be automatic to **extend** a PF from $2^2$ to $2^3$ to evaluate quadratic effects
...
It is therefore preferable to perform centered PFs with **rotational symmetry around the origin**
...
:::columns
:::column
Suppose we consider **only** the opposite vertices of the PF in the figure: $(1), ab, ac, bc$
...
* How to choose fractionation for larger dimensions?
...
## Choosing fractionation: defining relationships
...
These relations are called **defining relations** because they define the FFP. It makes no difference whether you choose the positive or the negative half
:::
...
:::aside
The effects matrix for a fractional FP $2^{n-1}$ plus the defining relation uniquely identify a fractional factorial plan
:::


## *Alias* and information lost

In a fractional PF $2^{3-1}$ with defining relation $I=ABC$, we consider these effects:
$$
\begin{align}
...
Therefore, by splitting a PF you lose information: you lose the ability to discriminate between *aliased* effects. It is clear that the longer the defining relationship, the higher the degree of alias interactions with the direct effects will be (e.g. $A=BCDEF$)
...
Consequently, an alias $A=BCDEF$ can be neglected by assuming the significance of $A$ rather than that of $BCDEF$ by virtue of the Principle
...
For these two DRs there is a third, dependent one: $I=ABDE$. Any two of these three DRs are equivalent
...
* Count the number of letters in the $(p+1)$-tuple (previous example: $\left<5,5,4\right>$)
...
It is very common to see DoEs only partially applied...

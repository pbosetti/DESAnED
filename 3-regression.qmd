...
$$
where $x_i$ are the physical (random) variables, called **regressors** or **predictors**, while the $c_i$ are the **parameters** (constant) of the model

- If $n=1$ there is a single predictor and the model is called **univariate**, otherwise it is called **multivariate**

- For example, $y=a+bx+cx^2$ is a univariate model with parameters $a,~b,~c$
...
If the $f(\cdot)$ is an univariate analytical function and linear in the coefficients, then we can express it as $\mathbf{A}\mathbf{k}=\mathbf{y}$, where
...
- even in the case of a **multivariate** $f(\cdot)$, if it is linear in the coefficients it is always possible to express it as $\mathbf{A}\mathbf{k}=\mathbf{y}$ and therefore solve it with the pseudo-inverse method
...
:::columns
:::column
**Only if I have a model $v=f(x, S)$** can I perform a regression
